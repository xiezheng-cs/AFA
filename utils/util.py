...
def get_optimier_and_scheduler(args, model_target, feature_criterions, step, logger):
    if args.reg_type == 'l2':
        optimizer = optim.SGD([{'params': model_target.parameters()}],
                                lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)

    elif args.reg_type == 'l2fe':
        optimizer = optim.SGD(model_target.classifier.parameters(), lr=args.lr,
                              momentum=args.momentum, weight_decay=args.weight_decay)
    else:
        if feature_criterions:
            if args.base_model_name in ['inception_v3', 'resnet101', 'resnet50']:
                fc_param_id = list(map(id, model_target.fc.parameters()))
                optimizer = optim.SGD([{'params': model_target.fc.parameters(), 'weight_decay': args.weight_decay},
                                       {'params': filter(lambda p: id(p) not in fc_param_id, model_target.parameters())},
                                       {'params': feature_criterions.parameters()}],
                                      lr=args.lr, momentum=args.momentum)
            elif args.base_model_name in ['mobilenet_v2']:
                optimizer = optim.SGD([{'params': model_target.classifier.parameters(), 'weight_decay': args.weight_decay},
                                       {'params': model_target.features.parameters()},
                                       {'params': feature_criterions.parameters()}],
                                      lr=args.lr, momentum=args.momentum)
            else:
                assert False, 'Wrong model name!!!'
        else:
            optimizer = optim.SGD([{'params': model_target.parameters()}], lr=args.lr, momentum=args.momentum)
    logger.info('optimizer={}'.format(optimizer))

    # lr_scheduler
    if args.reg_type != 'l2fe':
        if args.lr_scheduler == 'coslr':
            lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, args.num_epochs)
        ...
        lr_scheduler = ExponentialLR(optimizer, gamma=math.exp(math.log(0.1) / 6))
